...
/**
 * Administra las operaciones relacionadas con las transacciones (ventas, compras y devoluciones).
 * @param db - Instancia de la base de datos.
 */
export async function manageTransactions(db: Database): Promise<void> {
      ...
      switch (sellAction) {
        case "Sell Item": {
          try {
            ...
            if (
              isNaN(newSale.hunterID) ||
              isNaN(newSale.quantity) ||
              isNaN(newSale.goodID) ||
              isNaN(newSale.date)
            ) {
              throw new Error(
                "⚠️ Invalid input: Hunter ID, quantity, good ID and date atributes must be filled.",
              );
            }
            ...
            if (newQuantity <= 0) {
              ...
              console.log(
                `❌ ${good.name} were out of stock and was eliminated from the inventory.`,
              );
            } else {
      ...
      switch (purchaseAction) {
        case "Add a Purchase": {
          try {
            ...
            if (
              isNaN(newPurchase.merchantID) ||
              isNaN(newPurchase.quantity) ||
              isNaN(newPurchase.date) ||
              isNaN(newPurchase.goodID)
            ) {
              throw new Error(
                "⚠️ Invalid input: Merchant ID, quantity, date and good ID atributes must be filled.",
              );
            }
      ...
      switch (returnAction) {
        case "Add a Return": {
          try {
            ...
            const { quantity } = await inquirer.prompt([
              {
                ...
                validate: (input) => {
                  if (!input || isNaN(input) || input <= 0)
                    throw new Error("⚠️ Quantity must be a valid number greater than zero.");
                  return true;
                },
              },
            ]);
            ...
            db.updateGood(good.id, { quantity: newQuantity });
            console.log(
              `✅ Return registered: ${quantity}x ${good.name} returned to ${db.getMerchantByID(merchantId)?.name} on ${date}.`,
            );
          } catch (error) {
            if (error instanceof Error) {
        ...
        case "List Returns": {
          ...
          if (returns.length === 0) {
            ...
            const formattedReturns = returns.map((returnn) => ({
              ...
              Date: returnn.date,
              Returner:
                db.getHunterByID(returnn.customerId)?.name ||
                db.getMerchantByID(returnn.customerId)?.name ||
                "Unknown",
              ItemsReturned: returnn.itemsReturned
